...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        if current_user.email != 'amit@gmail.com':
            return abort(403)
...
def login():
    form=LoginForm()
    if form.validate_on_submit():
        connections = db.session.execute(db.select(User).where(User.email == form.data.get("email"))).scalar()
        if not connections:
            flash("Wrong email id try with diffrent email id")
        elif not check_password_hash(connections.password,form.password.data) :
            flash("wrong password plese try agin!")
        else:
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('show_post',post_id=post_id,form = comment_form))
